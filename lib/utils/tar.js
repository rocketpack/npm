...
function pack_ (targetTarball, folder, pkg, cb_) {
  ...
  lock(targetTarball, function (er, lockObject) {
    if (er) return cb(er)

    tarballLock = lockObject

    new Packer({ path: folder, type: "Directory", isDirectory: true })
      .on("error", function (er) {
        if (er) log.error("tar pack", "Error reading " + folder, er.toString())
        return cb(er)
      })
...
function unpack (tarball, unpackTarget, dMode, fMode, uid, gid, cb) {
  log.verbose("tar unpack", tarball, unpackTarget)
...
function unpack_ ( tarball, unpackTarget, dMode, fMode, uid, gid, cb_ ) {
  ...
  lock(tarball, function(er, lockObject) {
    if (er) return cb(er)

    tarballLock = lockObject

    lock(unpackTarget, function (er, lockObject) {
      if (er) return cb(er)

      targetLock = lockObject
